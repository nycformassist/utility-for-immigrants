...
// ✅ FIX: Removed GoogleGenAI import — API calls now go through /api/analyze serverless function
...
export default function App() {
  ...
  const handleInterviewComplete = async () => {
    ...
    // ✅ FIX: API call goes to serverless /api/analyze — GEMINI_API_KEY never in frontend bundle
    try {
      const narrativeStarters = NARRATIVE_STARTERS
        .filter(s => answers[s.id] === 'yes')
        .map(s => s.labels['EN']);
    ...
    const check = (key: string) => {
      if (answers[key]) {
        ...
        if (answers[key] === 'yes') {
          ...
          if (selectedProxies.length > 0) score++;
        }
      }
    };
  ...
  const generateHtml = () => {
    ...
    // ✅ FIX: Merriweather font, CSS text watermark (no external image dependency), relative paths only
    return `
      <!DOCTYPE html>
      <html lang="en">
      <head>
        ...
        <style>
          ...
          .header img { width: 5rem; height: 5rem; margin: 0 auto 1rem auto; object-fit: contain; display: block; }
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl w-full text-center mb-8">
        {/* ✅ FIX: Relative path /logo.png — no Windows absolute paths, no external imgur URLs */}
        <img src="/logo.png" alt="The Human Anchor Seal" className="w-24 h-24 mx-auto mb-6 object-contain" onError={(e) => { (e.target as HTMLImageElement).style.display = 'none'; }} />
