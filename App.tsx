...
<head>
  ...
  <style>
    ...
    .analysis-packet{position:relative;}
    .analysis-packet::before{content:'';position:fixed;top:0;left:0;width:100%;height:100%;background-image:url('/anubis_seal.png')!important;background-repeat:repeat!important;background-size:160px!important;opacity:0.06;pointer-events:none;z-index:0;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}
    @media print{-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;.analysis-packet::before{display:block!important;background-image:url('/anubis_seal.png')!important;background-repeat:repeat!important;background-size:160px!important;-webkit-print-color-adjust:exact!important;print-color-adjust:exact!important;}}
...
<body>
  <div class="analysis-packet">
  <div class="header">
    <img src="/anubis_seal.png" alt="Anubis Official Seal" onerror="document.body.innerHTML='<h1 style=font-family:monospace;color:red;padding:2rem>BRANDING_ASSET_MISSING: IP VERIFICATION FAILED.</h1>'" style="width:140px;height:auto;margin:0 auto 1rem;display:block;" />
  ...
  const openPrintablePacket = async () => {
    if (!sealReady) {
      throw new Error('BRANDING_ASSET_MISSING: IP VERIFICATION FAILED.');
    }
    setIsPrinting(true);
    try {
      ...
      // Wait for document load AND anubis_seal.png to fully decode
      await new Promise<void>(resolve => {
        printWindow.addEventListener('load', () => resolve(), { once: true });
        setTimeout(resolve, 1200); // fallback if load event doesn't fire
      });

      // Preload the seal image in the print window before allowing print dialog
      await new Promise<void>(resolve => {
        const sealCheck = printWindow.document.querySelector('img[src="/anubis_seal.png"]') as HTMLImageElement | null;
        if (!sealCheck) {
          printWindow.close();
          throw new Error('BRANDING_ASSET_MISSING: IP VERIFICATION FAILED.');
        }
        if (sealCheck.complete && sealCheck.naturalWidth > 0) {
          resolve();
        } else {
          sealCheck.addEventListener('load', () => resolve(), { once: true });
          sealCheck.addEventListener('error', () => {
            printWindow.close();
            throw new Error('BRANDING_ASSET_MISSING: IP VERIFICATION FAILED.');
          }, { once: true });
          setTimeout(resolve, 800); // hard decode fallback
        }
      });
  ...
  return (
    <div className="min-h-screen flex flex-col items-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-3xl w-full text-center mb-8">
        <img src="/anubis_seal.png" alt="Anubis Official Seal" className="mx-auto mb-6 object-contain"
      ...
      <div className="max-w-3xl w-full bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
        ...
        {stage === 'PACKET' && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.7 }} className="p-8 sm:p-12 text-center">
            <div id="branding-header" style={{ display: 'flex', justifyContent: 'flex-end', width: '100%', marginBottom: '1rem' }}>
              <img
                src="/anubis_seal.png"
                alt="Anubis Official Seal"
                style={{ width: '140px', height: 'auto' }}
                onLoad={() => setSealReady(true)}
                onError={() => {
                  throw new Error('BRANDING_ASSET_MISSING: IP VERIFICATION FAILED.');
                }}
              />
            ...
            <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
              <button onClick={openPrintablePacket} disabled={isPrinting || !sealReady}
                ...
                title={!sealReady ? 'BRANDING_ASSET_MISSING: IP VERIFICATION FAILED.' : undefined}>
                <Printer className="w-5 h-5" />
                <span>{isPrinting ? 'Preparing...' : !sealReady ? 'Verifying seal...' : ui.openPrint}</span>
              </button>
